...
Script below is specifically for target1 using a Ridge LASSO Elastic technique.
...
randr = RidgeCV(alphas=[1, .5, .01, .05, .001, .0005], scoring='r2', normalize=True).fit(X_sample, y_sample)
randr.alpha_

#%% Ridge CV analyzing different alpha levels based on results form prior cell
randr = RidgeCV(alphas=[.55, .5, .45], scoring='r2', normalize=True).fit(X_sample, y_sample)
randr.alpha_
...
elast = ElasticNet(alpha = 0.0001, normalize=True)
