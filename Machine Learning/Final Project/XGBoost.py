...
Script below is specifically for target1 using a XGBoost technique.
...
print(cv_results['test-error-mean'].min())
...
# each value and then narrow it down. I narrowed it down to this start.
gridsearch_params = [
...
# We get the best score with a max_depth of 9 and min_child_weight of 6, so let's update
# our params
params['max_depth'] = 9
...
print(accuracy_score(model.predict(dtest), y_test))

#%% Save model
model.save_model("xgboost_target1.model")
